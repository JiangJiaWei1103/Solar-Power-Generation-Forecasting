# type: ignore
"""
Scrape PV LightHouse data.
Author: JiaWei Jiang

Raw data is scraped from https://www.pvlighthouse.com.au/.
"""
import json
import os
...
def time2sec(t: List[str]) -> float:
    """Convert time to second-unit representation.
...
def main() -> None:
    df = pd.read_csv(os.path.join(RAW_DATA_PATH, "complete.csv"))
    ...
    df = pd.concat([df[PK], df_scraped], axis=1)

    # Dump processed data
    df_scraped.to_csv("./data/processed/pv.csv", index=False)
